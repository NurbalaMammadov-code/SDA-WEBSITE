...
document.addEventListener("DOMContentLoaded", function () {
  ...
  loadMoreBtn.addEventListener("click", function () {
    // Her defasında güncel gizli kartları bul
    const hiddenCards = document.querySelectorAll(".insight-card1.hidden");
    ...
    if (!isExpanded) {
      ...
      // Tüm fazladan kartları tekrar gizle
      const allCards = document.querySelectorAll(".insight-card1");
      allCards.forEach((card, index) => {
        if (index >= 6) { // İlk 6 kart açık kalsın
          card.classList.add("hidden");
        }
      });

      loadMoreBtn.innerHTML = 'Load more <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1V15M8 15L14 9M8 15L2 9" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
